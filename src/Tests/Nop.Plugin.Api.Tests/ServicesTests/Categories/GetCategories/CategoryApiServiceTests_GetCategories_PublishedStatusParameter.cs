...
namespace Nop.Plugin.Api.Tests.ServicesTests.Categories.GetCategories
{
    ...
    public class CategoryApiServiceTests_GetCategories_PublishedStatusParameter
    {
        ...
        [Test]
        public void WhenAskForOnlyThePublishedCategories_GivenCategoriesCollection_ShouldReturnOnlyThePublishedOnesSortedById()
        {
            // Arange
        ...
        [Test]
        public void WhenAskForOnlyTheUnpublishedCategories_GivenCategoriesCollection_ShouldReturnOnlyTheUnpublishedOnesSortedById()
        {
            // Arange
            var expectedCollection = _existigCategories.Where(x => !x.Published && !x.Deleted).OrderBy(x => x.Id);
